...
type Stage struct {
	...
	// Publish 指向交付物里的一个字段：它逐条列出**可以进前台**的事实。
	//
	// 交付物里本来就写满了事实（改了哪些文件、量到了什么），但前台一份都拿不到——
	// 不是话说得不好，是没有路。这个字段就是那条路：本段被接受的**那一刻**，
	// 由 harness 逐条校验、逐条发出去。
	//
	// 校验必须住在 harness 里。类型得在白名单内、该带的字段一个不许少，
	// 而"这件事到底成没成立"由**接受**本身回答——交付通过了出口条件，
	// 它声明的事实才算已确认。Agent 不必、也不许自己举手说"确定"。
